...
def rename_Vgene(Vname):  ## TRBV05-04*01-> TRBV5-4*01
    Vname = Vname.split(',')[0]
    Vname = Vname.split(' or ')[0]

    if Vname[Vname.find('-') + 1] == '0':  ## -04 -> -4
        Vname = Vname[:(Vname.find('-') +
                        1)] + Vname[(Vname.find('-') + 2):]
    if Vname[Vname.find('V') + 1] == '0':  ## TRBV05 -> TRBV5
...
df['Vgene'] = df['Vgene'].apply(rename_Vgene)
...
df = df.drop_duplicates(subset=['CDR3b', 'Antigen-epitope'], keep='first',).reset_index()
...
## Filter CDR3b only corresponds to one Antigen or epitope
# df = df[df['Antigen'] != 'Unknown']
# cdr3b_groups = df.groupby('CDR3b')
# antigen_counts = cdr3b_groups.size()
# single_antigen_cdr3b = antigen_counts[antigen_counts == 1].index.get_level_values(0)
# df = df[df['CDR3b'].isin(single_antigen_cdr3b)]
