...
class ERGOLightning(pl.LightningModule):
    ...
    def train_dataloader(self):
        with open('Samples/' + self.dataset + '_train.pickle', 'rb') as handle:
            train = pickle.load(handle)
    ...
    def val_dataloader(self):
        with open('Samples/' + self.dataset + '_test.pickle', 'rb') as handle:
...
def ergo_ii_experiment():
    ...
    early_stop_callback = EarlyStopping(monitor='val_auc', patience=3, mode='max')
    trainer = Trainer(gpus=[0], logger=logger, early_stop_callback=early_stop_callback, weights_save_path="./cdr3_vj")
    trainer.fit(model)
...
# python Trainer.py 10 0 vdjdb LSTM --use_vj --aa_embedding_dim 80 --lr 0.001
