...
class EmbeddingERGO(pl.LightningModule):
    ...
    def encode(self, tcrb_batch, pep_batch):
        # PEPTIDE Encoder:
...
if __name__ == "__main__":
    ...
    for i in range(25, 326, 50):
        tcrb_list = data_loader.collate("D:/TCR/random2/random_sample_" + str(i) + ".csv", "LSTM")
        tcrb_batch = encoder.forward(tcrb_list, None)
        tcrb_encoding = encoder.encode(tcrb_batch, None)
        tcrb_encoding_result = tcrb_encoding.detach().numpy()
        print(tcrb_encoding_result.shape)
        np.save("D:/TCR/cluster/random2/ERGO-II/" + str(i) + ".npy", tcrb_encoding_result)

    '''
    data_loader = Data_Loader()
    #tcrb_list, peptide = data_loader.collate("", "LSTM")
    tcrb_list = data_loader.collate("D:/TCR/TCRantigenData_detailed1.csv", "LSTM")
    encoder = EmbeddingERGO(tcr_encoding_model="LSTM", cat_encoding="binary")
    #tcrb_batch, pep_batch = encoder.forward(tcrb_list, peptide)
    tcrb_batch = encoder.forward(tcrb_list, None)
    #tcrb_encoding, pep_encoding = encoder.encode(tcrb_batch, pep_batch)
    tcrb_encoding = encoder.encode(tcrb_batch, None)
    tcr_encode_result = tcrb_encoding.detach().numpy()
    #pep_encode_result = pep_encoding.detach().numpy()
    print(tcrb_encoding.shape)
    #print(pep_encoding)
    np.save("D:/TCR/cluster/detailed1/ERGO-II.npy", tcr_encode_result)
    #np.save("D:/TCR/combined_dataset/ERGO-II_epi.npy", pep_encode_result)
    '''